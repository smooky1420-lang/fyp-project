...
import { Zap, Wallet, Wifi, Sun, PlusCircle, Gauge, Sparkles, ArrowRight } from "lucide-react";
        ...
        <section className="rounded-2xl bg-white ring-1 ring-slate-200 shadow-sm p-5">
          ...
          <ul className="space-y-2 list-none p-0 m-0">
            {recommendations.map((rec, idx) => (
              <li
                key={`${rec.title}-${idx}`}
                className="rounded-xl bg-slate-50 ring-1 ring-slate-200 px-4 py-3 text-sm"
              >
                <div className="font-medium text-slate-900">{rec.title}</div>
                <p className="mt-1 text-slate-600 text-xs leading-relaxed">{rec.description}</p>
              </li>
            ))}
          </ul>
        </section>
